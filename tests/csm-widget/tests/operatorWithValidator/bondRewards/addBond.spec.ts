      ...
      async ({ widgetService, csmSDK }) => {
        ...
        await test.step('Verify information about bond', async () => {
          ...
          await test.step('Verify bond balance', async () => {
            ...
            await expect(
              bondRewardsPage.addBond.titledAmountBalance,
            ).toHaveText(`${bondSummary.excess.toCut(4)} stETH`);
          });
      ...
      test(
        ...
        async ({ widgetService, ethereumSDK }) => {
          ...
          await test.step(`Choose ${tokenName} symbol for bond`, async () => {
            const bondToken =
              bondRewardsPage.addBond.selectBondToken(tokenName);
            await bondToken.click();
          });

          const expectedBalance =
            await ethereumSDK.getBalanceByToken(tokenName);
      ...
      test(
        ...
        async ({ widgetService, ethereumSDK }) => {
          ...
          const expectedTokenAmount =
            `${tokenAmountToClaim * rateToStETH}`.toCut(4);

          await bondRewardsPage.addBond.amountInput.fill(
            `${tokenAmountToClaim}`,
          );
