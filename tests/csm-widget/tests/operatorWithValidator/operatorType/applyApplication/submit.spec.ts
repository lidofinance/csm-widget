...
import { ApplicationForm } from '../../../../pages/tabs/operatorType/applicationForm.page';
import { qase } from 'playwright-qase-reporter/playwright';
import {
  applicationStatus,
  applyApplicationMockResponse,
} from '../../../../../shared/services/mockResponses/applyApplication.mock';

const secretPhrase = generateMnemonic(english, 128);
test.use({ secretPhrase: secretPhrase });

test.describe('Operator with keys. ICS. Apply application. Submit', async () => {
  ...
  test(
    qase(
      378,
      'Should correctly show applicatioin status for pending only mainAddress',
    ),
    async ({ widgetService, secretPhrase, httpMockerService }) => {
  ...
  test(
    qase(
      379,
      'Should correctly show applicatioin status for pending with additional address',
    ),
    async ({ widgetService, secretPhrase, httpMockerService }) => {
      applyApplicationMockResponse.form.mainAddress =
        mnemonicToAccount(secretPhrase).address;
      ...
      await test.step('Check all additional addresses', async () => {
        for (const [
          ...
          await test.step(`Check additional address ${additionalAddress} #${i + 1}`, async () => {
            ...
            await expect(
              additionalAddressField.locator('input'),
            ).toBeDisabled();
          });
        }
      });
  ...
  test(
    qase(
      380,
      'Should correctly show applicatioin status for pending with socials links',
    ),
    async ({ widgetService, secretPhrase, httpMockerService }) => {
      applyApplicationMockResponse.form.mainAddress =
        mnemonicToAccount(secretPhrase).address;
  ...
  test(
    qase(381, 'Should correctly show applicatioin status for rejected'),
    async ({ widgetService, secretPhrase, httpMockerService }) => {
      applyApplicationMockResponse.form.mainAddress =
        mnemonicToAccount(secretPhrase).address;
  ...
  test(
    qase(382, 'Should correctly show proofs with points'),
    async ({ widgetService, secretPhrase, httpMockerService }) => {
      applyApplicationMockResponse.form.mainAddress =
        mnemonicToAccount(secretPhrase).address;
      applyApplicationMockResponse.status = applicationStatus.REJECTED;
  ...
  test(
    qase(383, 'Should correctly show proofOfExperience info'),
    async ({ widgetService, secretPhrase, httpMockerService }) => {
      applyApplicationMockResponse.form.mainAddress =
        mnemonicToAccount(secretPhrase).address;
      applyApplicationMockResponse.status = applicationStatus.REJECTED;
  ...
  test(
    qase(384, 'Should correctly show proofOfHumanity info'),
    async ({ widgetService, secretPhrase, httpMockerService }) => {
      applyApplicationMockResponse.form.mainAddress =
        mnemonicToAccount(secretPhrase).address;
      applyApplicationMockResponse.status = applicationStatus.REJECTED;
  ...
  test(
    qase(385, 'Should correctly show proofOfEngagement info'),
    async ({ widgetService, secretPhrase, httpMockerService }) => {
      applyApplicationMockResponse.form.mainAddress =
        mnemonicToAccount(secretPhrase).address;
      applyApplicationMockResponse.status = applicationStatus.REJECTED;
