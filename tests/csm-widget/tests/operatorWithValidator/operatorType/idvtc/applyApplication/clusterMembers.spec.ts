...
test.describe('Operator with keys. IDVTC. Apply application. Cluster members', async () => {
  ...
  test('Should fill all cluster member addresses', async ({
    widgetService,
  }) => {
  ...
  test('Should generate and copy the message to sign', async ({
    widgetService,
    secretPhrase,
  }) => {
    const applyForm = widgetService.operatorType.dvtApplicationForm.applyForm;
    ...
    await test.step('Copy the message to sign', async () => {
      ...
      expect(clipboard).toBe(clusterMemberMessage(memberAddress, mainAddress));
    });
  });

  test('Should toggle the confirmation checkbox', async ({ widgetService }) => {
    const applyForm = widgetService.operatorType.dvtApplicationForm.applyForm;
  ...
  test('Should reject an invalid signature', async ({
    ...
    await test.step('Sign with a different account than the address', async () => {
      ...
      const message = clusterMemberMessage(memberAccount.address, mainAddress);
  ...
  test('Should require ICS-approved cluster addresses', async ({
    widgetService,
  }) => {
    const applyForm = widgetService.operatorType.dvtApplicationForm.applyForm;
    const member = applyForm.getClusterMember(0);
  ...
  test('Should not verify a member when its address is not ICS-approved', async ({
    widgetService,
    secretPhrase,
  }) => {
    const applyForm = widgetService.operatorType.dvtApplicationForm.applyForm;
  ...
  test('Should reject duplicate cluster addresses', async ({
    widgetService,
  }) => {
    const applyForm = widgetService.operatorType.dvtApplicationForm.applyForm;
  ...
  test('Should accept optional member contacts', async ({ widgetService }) => {
    const applyForm = widgetService.operatorType.dvtApplicationForm.applyForm;
    const member = applyForm.getClusterMember(0);
