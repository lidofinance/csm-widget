...
test.describe(
  ...
  async () => {
    ...
    test(
      qase(478, 'Should keep entered values after reload'),
      async ({ widgetService }) => {
        const applyForm =
        ...
        const address0 = mnemonicToAccount(
          generateMnemonic(english, 128),
        ).address;
        const address1 = mnemonicToAccount(
          generateMnemonic(english, 128),
        ).address;
    ...
    test(
      qase(459, 'Should keep the ICS error after reload for one member'),
      async ({ widgetService }) => {
        const applyForm =
          widgetService.operatorType.dvtApplicationForm.applyForm;
        const member = applyForm.getClusterMember(0);
        const address = mnemonicToAccount(
          generateMnemonic(english, 128),
        ).address;
    ...
    test(
      qase(460, 'Should keep ICS errors after reload for multiple members'),
      async ({ widgetService }) => {
        const applyForm =
          widgetService.operatorType.dvtApplicationForm.applyForm;
