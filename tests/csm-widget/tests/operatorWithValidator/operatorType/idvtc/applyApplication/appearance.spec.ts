...
test.describe('Operator with keys. IDVTC. Apply application. Appearance', async () => {
  ...
  test('Should show main address section texts', async ({ widgetService }) => {
  ...
  test('Should show socials section texts', async ({ widgetService }) => {
    const applyForm = widgetService.operatorType.dvtApplicationForm.applyForm;
    ...
    await test.step('Socials heading and description', async () => {
      ...
      ).toHaveAttribute('href', 'https://www.youtube.com/watch?v=yUX34iCbCWE');
    });
    ...
    await test.step('Discord section', async () => {
      ...
      await expect(
        applyForm.discordSection.getByRole('link', { name: 'the CSM channel' }),
      ).toHaveAttribute(
        'href',
  ...
  test('Should show cluster members section texts', async ({
    widgetService,
  }) => {
    const applyForm = widgetService.operatorType.dvtApplicationForm.applyForm;
  ...
  test('Should show confirmation and submit texts', async ({
    widgetService,
  }) => {
    const applyForm = widgetService.operatorType.dvtApplicationForm.applyForm;
