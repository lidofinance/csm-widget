...
test.describe(
  ...
  async () => {
    ...
    test(
      qase(363, 'Check main address appearance'),
    ...
    test(
      qase(364, 'Should add maximum 5 additional addresses'),
      async ({ widgetService }) => {
        const applicationForm = widgetService.operatorType.applicationForm;
    ...
    test(
      qase(365, 'Check additional addresses appearance'),
      async ({ widgetService }) => {
        const applicationForm = widgetService.operatorType.applicationForm;
        ...
        const additionalAddressField0 =
          applicationForm.submitApplicationForm.getAdditionalAddressFieldByIndex(
            0,
          );
    ...
    test(
      qase(366, 'Should successfully verify signature'),
      async ({ widgetService, secretPhrase }) => {
        const applicationForm = widgetService.operatorType.applicationForm;
        const additionalAddress = mnemonicToAccount(
          generateMnemonic(english, 128),
        ...
        await test.step('Check verified state for address', async () => {
          ...
          await expect(
            additionalAddressField0.verifiedAddressInput,
          ).toBeDisabled();
          await expect(
            additionalAddressField0.verifiedAddressInput,
          ).toHaveValue(additionalAddress.address);
        });
      },
    );

    test(
      qase(367, 'Should copy message to sign after click to copy button'),
      async ({ widgetService }) => {
        const applicationForm = widgetService.operatorType.applicationForm;
        const additionalAddress = mnemonicToAccount(
          generateMnemonic(english, 128),
    ...
    test(
      qase(368, 'Should open sign link in a new tab'),
      async ({ widgetService }) => {
        const applicationForm = widgetService.operatorType.applicationForm;
        const additionalAddress = mnemonicToAccount(
          generateMnemonic(english, 128),
