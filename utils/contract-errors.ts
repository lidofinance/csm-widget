...
> = {
  ...
  WrongPubkeyLength:
    'A validator key has an invalid length. Please verify the uploaded keys.',
  KeysLimitExceeded:
  ...
  // Top-up queue (CSM)
  TopUpQueueIsFull:
    'The top-up queue is currently full. Please try again later.',
...
// decodable revert or the name is intentionally unmapped. Accepts a pre-decoded
// revert so the abi decode runs ONCE per error (see resolveError); decodes
// internally when called without it.
export const getContractErrorCopy = (
  error: unknown,
...
// Accepts a pre-decoded revert to avoid re-decoding (see resolveError).
export const trackUnmappedContractError = (
  error: unknown,
  decoded = getDecodedRevert(error),
): void => {
