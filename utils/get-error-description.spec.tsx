...
it('returns undefined description when SOMETHING_WRONG error has no message', () => {
  // Plain object with no recognizable code/message → SOMETHING_WRONG, and
  // extractErrorMessage returns undefined, so there is no description.
  const { code, description } = resolveError({ foo: 'bar' });
  expect(code).toBe(ErrorCode.SOMETHING_WRONG);
