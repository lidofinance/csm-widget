...
describe('getContractErrorCopy', () => {
  ...
  it('uses a pre-decoded revert when provided (no re-decode)', () => {
    // The error object itself carries no revert; the copy comes solely from the
    // threaded `decoded` arg — proving resolveError can decode once and pass it.
    expect(
      getContractErrorCopy(new Error('opaque'), {
        name: 'BondLockNotExpired',
        args: [],
      } as any),
    ).toMatch(/bond is still locked/i);
  });
});
...
describe('trackUnmappedContractError', () => {
  ...
  it('uses a pre-decoded revert when provided (no re-decode)', () => {
    trackUnmappedContractError(new Error('opaque'), {
      name: 'SomeOracleInternalError',
      args: [],
    } as any);
    expect(trackMatomoError).toHaveBeenCalledWith(
      'SomeOracleInternalError',
