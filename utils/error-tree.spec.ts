...
describe('extractDataMessage', () => {
  it('reads the first data[] element message (Ledger-live shape)', () => {
    expect(
      extractDataMessage({ data: [{ message: 'Transaction rejected' }] }),
    ).toBe('Transaction rejected');
  });

  it('returns undefined when data is not an array or has no message', () => {
    expect(extractDataMessage({ data: 'nope' })).toBeUndefined();
    expect(extractDataMessage({ data: [{}] })).toBeUndefined();
    expect(extractDataMessage({ data: [] })).toBeUndefined();
    expect(extractDataMessage(null)).toBeUndefined();
  });
});
