...
export declare class AccountingSDK extends CsmSDKModule {
  protected get contract(): GetContractReturnType<
    ...
    WalletClient
  >;
  protected get parametersContract(): GetContractReturnType<
    typeof CSParametersRegistryAbi,
    WalletClient
  >;
  getBondAmountByKeysCountETH({
    curveId,
    keysCount,
  }: BondAmountByKeysCountProps): Promise<bigint>;
  ...
  getKeysCountByBondAmount({
    amount,
    curveId,
  }: KeysCountByBondAmountProps): Promise<bigint>;
  getBondForNextKeysETH({
    nodeOperatorId,
    keysCount,
  }: BondForNextKeysProps): Promise<bigint>;
  getBondForNextKeysWstETH({
    nodeOperatorId,
    keysCount,
  }: BondForNextKeysProps): Promise<bigint>;
