...
let PermissionlessGateSDK = (() => {
    ...
    return _a = class PermissionlessGateSDK extends _classSuper {
            ...
            async addNodeOperatorStETH(props) {
                ...
                const { hash, permit } = await this.getPermit({ token: index_js_3.TOKENS.steth, amount, ...rest }, _permit);
                if (hash)
                    return { hash };
                const args = [
                    keysCount,
            ...
            async addNodeOperatorWstETH(props) {
                const { amount, keysCount, publicKeys, signatures, managementProperties, referrer, permit: _permit, ...rest } = await this.parseProps(props);
                const { hash, permit } = await this.getPermit({ token: index_js_3.TOKENS.wsteth, amount, ...rest }, _permit);
                if (hash)
                    return { hash };
                const args = [
                    keysCount,
            ...
            async getPermit(props, preparedPermit) {
                if (preparedPermit)
                    return { permit: (0, index_js_4.stripPermit)(preparedPermit) };
                const result = await this.bus?.get('spending')?.signPermitOrApprove(props);
                return {
                    hash: result?.hash,
                    permit: (0, index_js_4.stripPermit)(result?.permit ?? index_js_3.EMPTY_PERMIT),
                };
            }
